...
| 18 | UI 布局 | 竖屏→横屏三栏 Switch 风格 | 匹配 ELF2 7寸 1024×600 屏比例 |
| 19 | IP 角色 | QPainter 绘制卡通狐狸 | Qt5 Linux 不支持彩色 emoji 渲染 |
| 20 | 词汇追踪驱动 | 中文焦点词 → AI 翻译 → 英文追踪 | 替代随机 CET 词匹配，精准定位学习目标 |
| 21 | AI 说话追踪 | 恢复追踪（target 事件） | 用户用中文问 → AI 翻译引入英文词 → 面板展示 |
| 22 | 代码管理 | Git + GitHub + 分支策略 | main=稳定, feature 分支=实验, 代码回流保护 |

---

## Phase 8 — UI 重设计 + 中文焦点词重构 (2026-05-26)

### 决定：横屏三栏布局（Switch 风格）

**当时情况**：原 500×700 竖屏窗口适合 PC 开发调试，但 ELF2 7 寸屏是 1024×600 横向比例。

**选择**：横屏三栏——左栏 IP 角色(30%)、中栏对话(45%)、右栏词汇追踪(25%)。参考 Gemini 产出的 HTML 设计稿翻译为 Qt Widgets。

### 决定：IP 角色用 QPainter 绘制

**问题**：Qt5 在 Linux 上不支持彩色 emoji 渲染。QLabel 放 🦊 不显示。

**选择**：用 QPainter 绘制卡通狐狸头像（圆形脸+三角耳朵+眼睛高光），纯代码实现，不依赖字体文件和图片。

### 决定：词汇追踪从"CET 词库被动匹配"改为"中文焦点词驱动"

**问题**：原方案扫描对话中所有 CET 词汇，导致大量无关词进入面板（ahead/mind/get）。用户真正想学的词无法突出。

**新流程**：
1. 提取用户消息中的中文词 → 作为 Prompt 焦点
2. Prompt 强制要求 AI "先完整转述用户的中文意思，再回答"
3. 从 AI 回复中提取英文 CET 词 → 发射 target 事件
4. 用户后续主动使用 → used → learning

### 决定：词汇面板标签简化

去掉"刚遇到"（橙色）和"薄弱词"（红色）。新标签：🟣 目标词、🟢 你用过、🔵 复习中。

### 已知问题（待修复）

1. **LC-ISSUE-1**: AI 回复词汇追踪范围过大——追踪了全部 CET 词，应只追踪中文翻译相关的那部分
2. **LC-ISSUE-2**: 同一词出现双重状态（目标词+你用过），需要去重合并
